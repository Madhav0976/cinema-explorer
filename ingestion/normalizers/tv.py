...
from ingestion.normalizers.movie import KEY_CREW_JOBS
...
def normalize_tv(
    ...
    """Normalize TMDB TV series API response into a validated NormalizedTitlePayload."""
    # 1. Date and precision
    release_date, precision = parse_date_and_precision(raw_data.get("first_air_date"))
    ...
    # Fallback to origin_country if production_countries empty
    for origin in raw_data.get("origin_country") or []:
        if isinstance(origin, str):
            code = origin.upper().strip()
            if code and code not in seen_countries:
                seen_countries.add(code)
                countries.append(CountryPayload(code=code, name=code))

    # 6. Seasons
    seasons: list[SeasonPayload] = []
    seen_seasons: set[int] = set()
    for s in raw_data.get("seasons") or []:
        if isinstance(s, dict) and s.get("id") and s.get("season_number") is not None:
            s_id = s["id"]
            if s_id not in seen_seasons:
                seen_seasons.add(s_id)
                air_date, _ = parse_date_and_precision(s.get("air_date"))
                seasons.append(
                    SeasonPayload(
                        tmdb_season_id=s_id,
    ...
    # 8. Alternate Titles (TV uses 'results')
    alt_data = raw_data.get("alternative_titles") or {}
    alt_list = alt_data.get("results") or alt_data.get("titles") or []
    ...
    return NormalizedTitlePayload(
        tmdb_id=raw_data["id"],
        type="tv",
        title=raw_data.get("name") or raw_data.get("original_name") or "Untitled",
